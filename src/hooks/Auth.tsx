...
const authConnector = {
    ...
    login(data: loginForm, cb: (user: User) => void) {
...
export type AuthContextType = {
    user: User | null;
    login: (data: loginForm, cb: () => void) => void;
...
function useProvideAuth() {
    ...
    const login = (data: loginForm, cb: () => void) => {
        return authConnector.login(data, (user) => {
            setUser(user);
            localStorage.setItem("user", JSON.stringify(user));
            localStorage.setItem("jwt", user.jwt)
            localStorage.setItem("decryptedPrivateKey", symmetry_object_decryption(data.password, user.encryptedPrivateKey));
            cb();
        });
    };
